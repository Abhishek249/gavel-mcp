...
sandbox-q2755:
	$(BIN)/gavel sandbox q2755-dual-write \
...
validate-mr:
	...
	@set -a && . ./.env && set +a && \
		...
		$(BIN)/gavel validate-manual-report $${GAVEL_MR_ID:-$${PROOFLINE_MR_ID:-$(MR_ID)}} --sandbox $(SANDBOX)

collect-mr:
	@test -f .env || (echo "Missing .env — run: cp .env.example .env"; exit 1)
	@set -a && . ./.env && set +a && \
		test -n "$$PGPASSWORD" || (echo "Set PGPASSWORD in .env"; exit 1) && \
		$(BIN)/gavel collect-manual-report $${GAVEL_MR_ID:-$${PROOFLINE_MR_ID:-$(MR_ID)}}
